...
const CAROUSEL_INTERVAL = 8000; // 8 secondes

// Variables globales pour le carrousel
let globalGoToSlide = null;

// Fonctions globales du carrousel
function startCarousel() {
  // Arrêter le carrousel existant s'il y en a un
  if (carouselInterval) {
    clearInterval(carouselInterval);
  }
  
  // console.log('PlastoBlast: Démarrage du carrousel automatique');
  
  carouselInterval = setInterval(() => {
    const slides = document.querySelectorAll('.hero-carousel-slide');
    // console.log(`PlastoBlast: Vérification carrousel - ${slides.length} slides trouvées`);
    
    if (slides.length > 0 && globalGoToSlide) {
      const nextSlide = (currentSlide + 1) % slides.length;
      // console.log(`PlastoBlast: Passage à la slide ${nextSlide + 1}/${slides.length}`);
      globalGoToSlide(nextSlide);
    } else {
      // console.warn('PlastoBlast: Aucune slide trouvée ou fonction goToSlide non disponible');
      // console.log('PlastoBlast: globalGoToSlide:', globalGoToSlide);
    }
  }, CAROUSEL_INTERVAL);
}
...
function initHeroCarousel() {
  ...
  if (!track || !slides.length || !indicators.length) {
    // console.warn('PlastoBlast: Éléments du carrousel non trouvés');
    return;
  }

  // Stocker les dimensions des images pour l'adaptation dynamique
  const imageDimensions = [];
  let imagesLoaded = 0;

  // Fonction pour adapter le cadre aux dimensions de l'image
  function adaptFrameToImage(slideIndex) {
    if (imageDimensions[slideIndex] && carousel && carouselContainer) {
      const { width, height, ratio } = imageDimensions[slideIndex];
      const maxWidth = 500; // Largeur maximale du carrousel
      const minWidth = 320; // Largeur minimale augmentée
      const minHeight = 280; // Hauteur minimale augmentée
      const maxHeight = 400; // Hauteur maximale pour éviter les carrousels trop hauts
